...
public class DictCSVWriter extends DictWriter {
	private class DictInfo {
		...
		TreeMap<SchemaAttribute, Integer> num_attr_columns =
			new TreeMap<SchemaAttribute, Integer>();
	}
	...
	public void write() throws IOException {
			...
			if (ec != null) {
				...
				for (int i=0; i<attrs.length; i++) {
					if (attrs[i] instanceof ArraySchemaAttribute) {
						ArraySchemaAttribute asa =
							(ArraySchemaAttribute) attrs[i];
						Object[] values = (Object[]) asa.getValue();
						if (! info.num_attr_columns.containsKey(attrs[i]))
							info.num_attr_columns.put(
								attrs[i], values == null ? 1 : values.length);
						else
		...
		for (Iterator<SchemaAttribute> it =
			info.num_attr_columns.keySet().iterator(); it.hasNext(); ) {
	...
	private void write_entry(Writer writer, DictInfo info, Entry entry)
		...
		for (int i=0; i<info.num_senses; i++) {
			writer.write(i < senses.length ?
					getField(senses[i].getTranslation()) : getField(""));
			writer.write(_field_delimiter);
			writer.write(i < senses.length ?
					getField(senses[i].getDefinition()) : getField(""));
			writer.write(_field_delimiter);
		}
		...
		for (int i=0; i<info.num_categories; i++) {
			writer.write(i < categories.length ?
					getField(categories[i]) : getField(""));
			writer.write(_field_delimiter);
		}
		...
		for (int i=0; i<info.num_mmfiles; i++) {
			writer.write(i < mmfiles.length ?
					getField(mmfiles[i]) : getField(""));
			writer.write(_field_delimiter);
		}
		...
		if (ec != null) {
			Iterator<SchemaAttribute> it =
				info.num_attr_columns.keySet().iterator();
			while (it.hasNext()) {
				...
				if (attr instanceof ArraySchemaAttribute) {
					...
					int num_cols = info.num_attr_columns.get(attr);
					for (int i=0; i<num_cols; i++) {
						writer.write(getField(vals != null && i<vals.length ?
								vals[i].toString() : ""));
						writer.write(_field_delimiter);
					}
				} else {
