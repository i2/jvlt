...
public abstract class AbstractModel {
	...
	/**
	 * This method sets the number of executed actions to zero while the
	 * list of actions is not cleared. This causes the method
	 * <i>isDataModified()</i> to return <i>false</i>, but it is still possible
	...
	protected void fireEvent (Object obj) {
		if (obj instanceof UndoableActionEvent)	{
			...
			Iterator<UndoableActionListener> it =
				_undoable_action_listeners.iterator();
			while (it.hasNext())
				it.next().actionPerformed(event);
		} else if (obj instanceof ModelResetEvent) {
			ModelResetEvent event = (ModelResetEvent) obj;
			Iterator<ModelResetEventListener> it =
				_reset_event_listeners.iterator();
			while (it.hasNext())
