...
public class DictImporter {
	...
	/**
	 * Determine the entries that can be imported.
	 * Only entries that do not already exist in the dictionary
	 * will be imported.
	...
	/**
	 * Determine the examples that can be added to the dictionary.
	 * Examples that already exist in the dictionary cannot be imported.
	 * The same is true for examples whose linked entries cannot be imported.
	 * @param dict The dictionary that will be imported.
	 * @param entries The entries that will be imported. They can be obtained
	 *   by the method {@link #getImportedEntries(Dict)}.
	 */
	public Collection<Example> getImportedExamples(
			Dict dict, Collection<Entry> entries) {
		TreeSet<Entry> entry_set = new TreeSet<Entry>();
		entry_set.addAll(entries);
		TreeSet<Example> new_examples
			= new TreeSet<Example>(new Example.Comparator());
	...
	public void importDict(Dict dict) throws DictException {
		...
		ImportAction ia = new ImportAction(old_lang, new_lang,
			entries, examples);
