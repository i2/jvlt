...
public abstract class AbstractEntryDialog extends AbstractDialog {
	...
	protected class ComponentReplacementHandler implements
	...
	protected class ActionHandler implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand().equals("add")) {
				...
				SenseDialogData data = new SenseDialogData(_current_entry,
						sense);
				...
				if (orig_sense.getParent() != null) {
					Collection<Example> linked_examples = _model.getDict()
							.getExamples(orig_sense);
				...
				if (result == JOptionPane.YES_OPTION) {
					RemoveSenseAction action = new RemoveSenseAction(_sense_map
							.get(sense), index);
					_sense_actions.add(action);
				...
				CustomDialog.showDialog(getAdvancedDialogData(),
						getContentPane(), GUIUtils.getString("Labels",
								"advanced"));
			}
		}
	}
	...
	protected void updateComponents() {
		...
		if (_current_entry != null) {
			...
			_pronunciation_editor.setSelectedItems(_current_entry
					.getPronunciations());
		} else {
			_orth_field.setText("");
	...
	protected void updateEntries() throws InvalidDataException {
		...
		_current_entry.setPronunciations(Utils
				.objectArrayToStringArray(_pronunciation_editor
						.getSelectedItems()));
	...
	private void initUi() {
		...
		_pronunciation_editor
				.addComponentReplacementListener(new ComponentReplacementHandler());
		...
		sense_panel.setBorder(new TitledBorder(new EtchedBorder(
				EtchedBorder.LOWERED), GUIUtils.getString("Labels", "senses")));
	...
	private void moveSelectedElement(int direction) {
		...
		Sense sense = (Sense) obj;
		MoveSenseAction action = new MoveSenseAction(_sense_map.get(sense),
				index, new_index);
		_sense_actions.add(action);
	}
}
