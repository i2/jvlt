...
public class ObjectSelectionPanel extends JPanel {
	...
	public Object[] getSelectedObjects() { return _selected_objects.toArray(); }
	
	/**
	 * Adds the set of selected objects.
	 * Call {@link #setSelectedObjects} after {@link #setAvailableObjects}. */
	...
	protected void updateButtonPanel() {
		if (_allow_reordering)
			_button_panel.setButtons(new JButton[] {
				new JButton(_up_action), new JButton(_down_action),
				new JButton(_left_action), new JButton(_right_action)});
		else
			_button_panel.setButtons(new JButton[] {
				new JButton(_left_action), new JButton(_right_action)});
	}
