...
public class CSVImportPanel extends CSVPanel {
	...
	private class ChangeHandler implements ChangeListener {
		public void stateChanged(ChangeEvent ev) { _preview_model.update(); }
	}
	...
	private class PreviewTableModel extends AbstractTableModel {
		...
		public int getRowCount() { return _column_names.length; }
		...
		private String[] getColumnNames() {
			...
			int num_senses =
				_num_senses_spinner_model.getNumber().intValue();
			int num_categories =
				_num_categories_spinner_model.getNumber().intValue();
			int num_mmfiles =
				_num_multimedia_files_spinner_model.getNumber().intValue();
			int num_examples =
				_num_examples_spinner_model.getNumber().intValue();
			AttributeTable.AttributeTableModel tablemodel =
				(AttributeTable.AttributeTableModel)_attribute_table.getModel();
	...
	protected void initComponents() {
		...
		_num_examples_spinner = new LabeledSpinner(
			_num_examples_spinner_model);
		_num_examples_spinner.setLabel("num_examples");
		_num_examples_spinner.addChangeListener(ch);
			
		_ignore_first_row_box = new JCheckBox(
			GUIUtils.getString("Actions", "ignore_first_row"));
		...
		_attribute_pane.setBorder(new TitledBorder(
			new EtchedBorder(EtchedBorder.LOWERED),
			GUIUtils.getString("Labels", "attributes")));
		...
		_preview_pane.setBorder(new TitledBorder(
			new EtchedBorder(EtchedBorder.LOWERED),
			GUIUtils.getString("Labels", "csv_columns")));
	}
	...
	public SchemaAttribute[] getAttributes() {
		AttributeTable.AttributeTableModel model =
			(AttributeTable.AttributeTableModel) _attribute_table.getModel();
	...
	public void loadState() {
		...
		_text_delim_box.setSelectedItem(
			config.getProperty("CSVImport.TextDelimiter", "\""));
	...
	public void saveState() {
		...
		config.setProperty("CSVImport.TextDelimiter",
			String.valueOf(getTextDelimiter()));
		config.setProperty("CSVImport.FieldDelimiter",
			String.valueOf(getFieldDelimiter()));
		config.setProperty("CSVImport.Charset",
			String.valueOf(getCharset()));
		config.setProperty("CSVImport.NumSenses",
			String.valueOf(getNumSenses()));
		config.setProperty("CSVImport.NumCategories",
			String.valueOf(getNumCategories()));
		config.setProperty("CSVImport.NumMultimediaFiles",
			String.valueOf(getNumMultimediaFiles()));
		config.setProperty("CSVImport.NumExamples",
			String.valueOf(getNumExamples()));
		config.setProperty("CSVImport.IgnoreFirstRow",
			String.valueOf(getIgnoreFirstRow()));
		SchemaAttribute[] attrs = getAttributes();
	...
	private void updateAttributePanel() {
		...
		if (language != null)
			_attribute_table.setAvailableAttributes(
				getSchemaAttributes(language));
	}
	...
	private SchemaAttribute[] getSchemaAttributes(String language) {
		...
		catch (XPathExpressionException ex) { return new SchemaAttribute[0]; }
	}
}
...
class AttributeTable extends JTable {
	public static class AttributeTableModel extends AbstractTableModel {
		...
		public int getRowCount() { return _attributes.size()+1; }
		...
		public void setValueAt(Object value, int row, int column) {
			if (row >= _attributes.size()) {
				...
				if (column == 0) {
					SchemaAttribute attr = (SchemaAttribute)
						_translation_attr_map.get(value);
					if (attr == null) {
		...
		public void setAvailableAttributes(SchemaAttribute[] attrs) {
			...
			for (int i=0; i<attrs.length; i++)
				_translation_attr_map.put(
					_resources.getString(attrs[i].getName()), attrs[i]);
		}
		...
		private SchemaAttribute getAttributeAt(int row) {
			...
			if (val == null || val.equals(""))
				...
				return (SchemaAttribute) _translation_attr_map.get(val);
		}
	}
...
class SpinnerEditor extends AbstractCellEditor implements TableCellEditor {
	...
	public Object getCellEditorValue() { return _spinner.getValue(); }
}
