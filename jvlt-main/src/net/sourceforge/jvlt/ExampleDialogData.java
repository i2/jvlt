...
public class ExampleDialogData extends CustomDialogData
	implements ActionListener, CaretListener, ListSelectionListener,
		TreeSelectionListener {
	private Example _example;
	private Dict _dict;
	private SimpleEntryFilter _entry_filter;
	/** Index of the first character in the current selection
	 *  (-1 if there is no selection). */
	private int _current_selection_start;
	/** Index of the first character after the current selection
	 *  (-1 if there is no selection). */
	...
	public void updateData() throws InvalidDataException {
		if (_example.getSenses().length == 0)
			throw new InvalidDataException(
				GUIUtils.getString("Messages", "no_links"));
		...
		try {
			...
			throw new InvalidDataException(GUIUtils.getString(
				"Messages", "invalid_html", new Object[]{ e.getMessage() }));
		}
				...
				if (obj instanceof Sense)
					...
					if (entry.getSenses().length > 1)
						MessageDialog.showDialog(_content_pane,
							...
							GUIUtils.getString("Messages", "select_meaning"));
					else if (entry.getSenses().length == 0)
						MessageDialog.showDialog(_content_pane,
							MessageDialog.WARNING_MESSAGE,
							GUIUtils.getString("Messages", "no_meaning"));
					else
	...
	private void init() {
		...
		preview_scrpane.setBorder(new TitledBorder(
			new EtchedBorder(EtchedBorder.LOWERED),
			GUIUtils.getString("Labels", "preview")));
		...
		_add_senses_panel.setBorder(new TitledBorder(
			new EtchedBorder(EtchedBorder.LOWERED),
			GUIUtils.getString("Labels", "create_link")));
		_add_senses_panel.setLayout(new GridBagLayout());
		CustomConstraints cc = new CustomConstraints();
		cc.update(0, 0, 1.0, 0.0);
		_add_senses_panel.add(new JLabel(GUIUtils.getString(
			"Messages", "add_senses_help")), cc);
		...
		senses_panel.setBorder(new TitledBorder(
			new EtchedBorder(EtchedBorder.LOWERED),
			GUIUtils.getString("Labels", "links")));
		...
		_content_pane.add(
			GUIUtils.getLabel("translation", _translation_field), cc);
	...
	private void updatePreviewPane() {
		...
		if (_current_selection_start<0 || _current_selection_end<0) {
			...
			String suffix = str.substring(
				_current_selection_end, str.length());
	...
	private void updateSelectedSensesTree() {
		...
		if (text == null || text.equals(""))
			...
			List<Entry> entries =
				_entry_filter.getMatchingEntries(_dict.getEntries());
			...
			_selected_senses_tree.setEntries(
				entry_list.toArray(new Entry[0]));
		}
	}
	...
	private void updateStatusLabels() {
		...
		if (tf == null)
			...
			if (example == null)
				num++;
			else if (! Utils.arrayContainsItem(
				example.getSenses(), tf.getSense()))
				num++;
...
//		System.out.println(fragment.getText());
		...
		if (_example != null) {
			...
			for (int i=0; i<fragments.length; i++) {
				...
				if (tf_first <= first_index && first_index <= tf_last) {
					...
					delete_last = last_index<=tf_last?
						last_index-tf_first:tf_last-tf_first;
				}
				...
				if (delete_first >= 0) {
					if (delete_first == tf_first && delete_last == tf_last)
						...
						tf.setText(Utils.removeSubstring(
							tf_str, delete_first, delete_last));
				}
	...
	public void setExample (Example example) { _example = example; }
}
