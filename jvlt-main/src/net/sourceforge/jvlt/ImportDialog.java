...
class ImportWizardModel extends DialogWizardModel {
	...
	public WizardPanelDescriptor nextPanelDescriptor(String command)
		...
		if (_current_descriptor instanceof StartImportDescriptor) {
			StartImportDescriptor sid =
				(StartImportDescriptor) _current_descriptor;
			if (command.equals(Wizard.NEXT_COMMAND)) {
				try {
					...
					throw new InvalidInputException(ex.getShortMessage(),
						ex.getLongMessage());
	...
	private void prepareImport(Dict dict) throws InvalidInputException {
		...
		if (newlang != null && oldlang != null && ! newlang.equals(oldlang))
			throw new InvalidInputException(
				GUIUtils.getString("Messages", "invalid_language"),
				"Invalid language: " + oldlang + "!=" + newlang);
...
class StartImportDescriptor extends WizardPanelDescriptor {
	...
	public Dict getDict() throws DictReaderException, IOException {
		...
		if (_type_box.getSelectedItem().equals(csv_file)) {
			...
			csv_reader.setNumMultimediaFiles(
				_csv_panel.getNumMultimediaFiles());
	...
	private void initUI() {
		Action open_action =
			GUIUtils.createTextAction(new ActionHandler(), "open");
		...
		_panel.add(
			new JLabel(GUIUtils.getString("Messages", "start_import")), cc);
...
class ImportResultsDescriptor extends WizardPanelDescriptor {
	...
	public void setEntries(
			Collection<Entry> imported, Collection<Entry> not_imported) {
		_entry_label.setText(getLabel(imported.size(),
			imported.size()+not_imported.size(), "num_words"));
		_imported_entries_list.setEntries(imported);
		_not_imported_entries_list.setEntries(not_imported);
	}

	public void setExamples(
			Collection<Example> imported, Collection<Example> not_imported) {
		_example_label.setText(getLabel(imported.size(),
			imported.size()+not_imported.size(), "num_examples"));
	...
	public void loadState() {
		_clear_stats_box.setSelected(
			JVLT.getConfig().getBooleanProperty("import_clear_stats", false));
	}
	
	public void saveState() {
		JVLT.getConfig().setProperty(
				"import_clear_stats", _clear_stats_box.isSelected());
	}
