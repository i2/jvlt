...
public class ExamplePanel extends JPanel implements ActionListener,
	...
	public void actionPerformed (ActionEvent e) {
		if (e.getActionCommand().equals("add")) {
			...
			int result = CustomDialog.showDialog(data, this,
				GUIUtils.getString("Labels", "add_example"));
			if (result == CustomDialog.OK_OPTION) {
				AddDictObjectAction action=new AddDictObjectAction(new_example);
			...
			if (_current_examples.size() > 0) {
				int result = JOptionPane.showConfirmDialog(this,
					GUIUtils.getString("Messages", "remove_examples"),
				...
				if (result == JOptionPane.YES_OPTION) {
					RemoveExamplesAction action = new RemoveExamplesAction(
						_current_examples);
					action.setMessage(
						GUIUtils.getString("Actions", "remove_example"));
					_model.getDictModel().executeAction(action);
				}
			}
	...
	public synchronized void dictUpdated(DictUpdateEvent event) {
		if (event instanceof ExampleDictUpdateEvent) {
			...
			if (eevent.getType() == ExampleDictUpdateEvent.EXAMPLES_ADDED) {
				...
				fireFilterEvent(new FilterEvent<Example>(
					this, _table_model.getObjects()));
	...
	private void init() {
		...
		_example_table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				Example example = (Example) _table_model.getObjectAt(index);
				editExample(example);
			}		
		});
	...
	private void updateActions() {
		boolean element_selected
			= (_example_table.getSelectedObjects().size() > 0);
...
abstract class Filter {
	...
	public Filter(String filter_string) { _filter_string = filter_string; }
	
	public Filter() { this(""); }
	
	public String getFilterString() { return _filter_string; }

	public void setFilterString(String val) { _filter_string = val; }
}

class ExampleFilter extends Filter {
	boolean _match_case = true;
	
	public ExampleFilter (String filter_string) { super(filter_string); }
	
	public ExampleFilter () { this (""); }
	...
	public Collection<Example> getMatchingExamples(
