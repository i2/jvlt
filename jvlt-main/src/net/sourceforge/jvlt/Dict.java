...
public class Dict extends Object {
	private static Entry.Comparator _entry_comparator
		= new Entry.Comparator();
	private static Example.Comparator _example_comparator
	...
	/**
	 * @throws DictException If an entry with the same primary key as
	 * the argument entry already exists in the dictionary. */
	...
	public int getEntryCount() { return _entry_set.size(); }
	...
	public Entry getEntry(Entry entry) {
		...
		if (tail.size() == 0)
			...
			Entry first = (Entry) tail.iterator().next();
	...
	public Collection<Entry> getEntries() { return _entry_set; }
	...
	public void addExample (Example example) throws DictException {
		if (_example_set.contains(example))
			throw new DictException("Example \""+example+"\" already exists.");
	...
	public int getExampleCount() { return _example_set.size(); }

	public Example getExample (String id) {
		if (_id_example_map.containsKey(id))
			return (Example) _id_example_map.get(id);
		else
			return null;
	}
	...
	public Collection<Example> getExamples() { return _example_set; }
	...
	public void setLanguage(String lang) throws DictException {
		...
		try {
			EntryAttributeSchemaReader reader =
				new EntryAttributeSchemaReader();
	...
	public EntryAttributeSchema getEntryAttributeSchema() { return _schema; }
}
