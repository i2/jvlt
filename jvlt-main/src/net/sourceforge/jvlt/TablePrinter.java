...
public class TablePrinter implements Printable {
	...
	public PageFormat getPageFormat() { return _format; }
	
	/**
	 * Returns the number of pages obtained by the last call of renderPages().*/
	public int getPageNumber() { return _page_number; }
	...
	public void paintPage(Graphics2D g2d, int page)
		...
		ColumnInfo info = getColumnInfo(_format, _vertical_spacing,
			_model.getColumnCount());
			
		Rectangle2D r2d = new Rectangle2D.Double();
		r2d.setRect(0, _format.getImageableY(),
			0, _format.getImageableHeight());
		for (int row=first_row; row<=last_row; row++) {
			double max_height = 0;
			for (int col=0; col<info.numCols; col++) {
				r2d.setRect(info.xOffsets[col], r2d.getY(),
					info.colWidths[col], r2d.getHeight());
				double height = renderCell(g2d, r2d,
					_model.getValueAt(row, col).toString(), true);
	...
	public void setPageFormat(PageFormat format) { _format = format; }
	
	/**
	 * Render pages using the current page format.
	 * @return Number of pages.
	 */
	public int renderPages(Graphics2D g2d)
		throws PrinterException {
		...
		for (int row=0; row<num_rows; row++) {
			...
			for (int col=0; col<num_cols; col++) {
				r2d.setRect(info.xOffsets[col], r2d.getY(),
					info.colWidths[col], r2d.getHeight());
				double height = renderCell(g2d, r2d,
					_model.getValueAt(row, col).toString(), false);
				if (height < 0) {
					// new page
					r2d.setRect(0,0,0,_format.getImageableHeight());
					pages.add(row-1);
	 ...
	 * @param paint Specifies whether the cell is painted or only its
	 *   size is calculated.
	 ...
	 */
	private double renderCell(Graphics2D graphics, Rectangle2D rect, String str,
		boolean paint) {
	...
	private ColumnInfo getColumnInfo(PageFormat format, double vertical_spacing,
		int num_cols) {
		...
		double total_width = format.getImageableWidth() -
			(num_cols-1)*vertical_spacing;
		for (int col=0; col<num_cols; col++) {
			if (col_percents[col] < 0)
				info.colWidths[col] = remaining_ratio/num_novalue*total_width;
			else
...
class LineBreaker {
	public static class Result {
		...
		public String[] getRows() {
			return (String[]) _rows.toArray(new String[0]);
		}
		...
		while (remaining.length() != 0)	{
			...
			if (r2d.getWidth() <= _width) {
				...
				while (true) {
					...
					Rectangle2D rec = metrics.getStringBounds(remaining,
						0, pos, graphics);
				...
				if (last_pos >= 0) {
					...
					int len = (int)
						(_width/r2d.getWidth()*remaining.length());
					Rectangle2D rec = metrics.getStringBounds(remaining,
						0, len, graphics);
					if (rec.getWidth() < _width) {
						int new_len = len+1;
						while (new_len < remaining.length()) {
							rec = metrics.getStringBounds(remaining,
								0, new_len, graphics);
							if (rec.getWidth() >= _width) {
								len = new_len-1;
								break;
