...
public abstract class ObjectListEditor {
	...
	public ObjectListEditor(String label) {
		...
		_more_action = GUIUtils.createTextAction(
			new ActionHandler(), "more_items");
	...
	public JComponent getInputComponent() {
		return _input_field==_single_input_field
			? _single_item_panel : _multi_item_panel;
	}
	...
	public void setLabel(String label) {
		...
		_single_item_label =
			GUIUtils.getLabel(label, _single_input_field.getComponent());
		_multi_item_label =
			GUIUtils.getLabel(label, _multi_input_field.getComponent());
		CustomConstraints cc = new CustomConstraints();
	...
	protected void replaceComponents(
		InputComponent old_component, InputComponent new_component) {
		...
		for (Iterator<ComponentReplacementListener> it=_listeners.iterator();
			...
			JPanel old_panel = old_component==_single_input_field
				? _single_item_panel : _multi_item_panel;
			JPanel new_panel = new_component==_single_input_field
				? _single_item_panel : _multi_item_panel;
			l.componentReplaced(
				new ComponentReplacementEvent(old_panel, new_panel));
		}
	}
